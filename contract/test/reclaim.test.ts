...
import { DappSmartContract, Reclaim, contracts } from "../src/types";
import {
  deployDappContract,
...
describe("Reclaim Tests", () => {
  ...
  describe("Proofs tests", async () => {
    ...
    it("should extract providerHash from context", async () => {
      ...
      const ctx =
        '{"contextAddress":"0x00000000000","contextMessage":"hi there","providerHash":"0x3246874620eacad8b93e3e05e5d5bb5877c9bed5ddcaf9b4f6cf291e0fb3c64e"}';
      const trgt = '"providerHash":"';
      const hash =
        "0x3246874620eacad8b93e3e05e5d5bb5877c9bed5ddcaf9b4f6cf291e0fb3c64e";
...
describe("Reclaim Witness Fetch Tests", () => {
  ...
  describe("Dapp Contract Tests", () => {
    let dapp: DappSmartContract;

    it.only("should deploy Dapp contract", async () => {
      let { contract, user, superProofs } = await loadFixture(proofsFixture);

      const erc20 = await deployMockERC20Token(ethers);
      console.log(erc20.address);

      dapp = await deployDappContract(
        contract,
        erc20.address,
        ethers,
        upgrades
      );
      dapp.setReclaimContract(contract.address);
      expect(dapp.address).to.not.equal(0);
      const reclaimAddress = await dapp.getReclaimAddress();

      // dappに対してerc20をmintする
      await erc20.mint(dapp.address, utils.parseEther("100"));

      //dappのerc20残高を確認する
      const balance = await erc20.balanceOf(dapp.address);
      expect(balance).to.equal(utils.parseEther("100"));

      console.log(balance);

      expect(reclaimAddress).to.equal(contract.address);

      await dapp.addScreenName("peaceandwhisky", utils.parseEther("0.1"));

      const test = superProofs[1];
      let x = await dapp.connect(user).submitProof(test);
      console.log(x);
